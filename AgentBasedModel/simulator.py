...
class Simulator:
    ...
    def __init__(
        ...
        # self.bids_history = dict()
        # self.asks_history = dict()
        # for exchange in exchanges:
        #     self.bids_history[exchange] = []  
        #     self.asks_history[exchange] = []  
    ...
    def simulate(self, n_iter: int, silent: bool = False) -> object:
        for it in tqdm(range(n_iter), desc='Simulation', disable=silent):
            ...
            # цикл по биржам, где я добавлю traded_counter в список и занулю его для следующей итерации
...
class SimulatorInfo:
    ...
    def __init__(
        ...
        self.orderbook_history = {idx: list() for idx in self.exchanges.keys()}  # echange: history of orderbook
        self.trades_history = {idx: list() for idx in self.exchanges.keys()}
        ...
        *Traders Statistics*
        ...
        for idx, exchange in self.exchanges.items():
            ...
            self.orders[idx].append({
                ...
                # },  # отсюда модно достать суммарный объем ордеров
                # 'volume mean': {
            ...
            self.orderbook_history[idx].append({
                'bid' : exchange.order_book['bid'].to_list(), 
                'ask' : exchange.order_book['ask'].to_list(),
                
            })

            self.trades_history[idx].append(
                exchange.exchnange_trades_history
            )
            exchange.exchnange_trades_history = []
    ...
    def orderbook_snapshot(self, idx : int = 0, tick : int = -1):
        """
        ...
        """
        order_book = self.orderbook_history[idx][tick]
        bids = pd.DataFrame(order_book['bid'])
        asks = pd.DataFrame(order_book['ask'])
        bids_asks = pd.concat([bids, asks], ignore_index = True)
        orderbook = pd.DataFrame(pd.DataFrame(bids_asks).groupby(['price','order_type'])['qty'].sum())

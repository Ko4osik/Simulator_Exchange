...
class Stock(Asset):
    ...
    """
    def __init__(self, dividend: float | int, restart: bool = True):
        """
...
class ExchangeAgent:
    ...
    def price(self) -> float:
        ...
        spread = self.spread()
        if spread:
            return round((spread['bid'] + spread['ask']) / 2, 3)
    ...
    def limit_order(self, order: Order):
        ...
        if order.order_type == 'bid':
            if order.price >= ask:
                prev_assets = order.trader.assets
                prev_cash = order.trader.cash
                order = self.order_book['ask'].fulfill(order, t_cost)
                next_assets = order.trader.assets
                next_cash = order.trader.cash
                traded_amount = next_assets - prev_assets
                if traded_amount > 0:
                    traded_price = (prev_cash - next_cash) / traded_amount
                    self.exchnange_trades_history.append({'qty': traded_amount, 'price' : traded_price, 'side' : 'buy', 'type' : 'limit_order', 'trader' : order.trader, 'exchange' : self.name})
            if order.qty > 0:
            ...
            if order.price <= bid:
                ...
                next_assets = order.trader.assets
                next_cash = order.trader.cash
                traded_amount = prev_assets - next_assets
                if traded_amount > 0:
                    traded_price = (next_cash - prev_cash) / traded_amount
                    self.exchnange_trades_history.append({'qty': traded_amount, 'price' : traded_price, 'side' : 'sell', 'type' : 'limit_order', 'trader' : order.trader, 'exchange' : self.name})
            if order.qty > 0:
    ...
    def market_order(self, order: Order) -> Order:
        ...
        if order.order_type == 'bid':
            ...
            if traded_amount > 0:
                traded_price = (prev_cash - next_cash) / traded_amount
                self.exchnange_trades_history.append({'qty': traded_amount, 'price' : traded_price, 'side' : 'buy', 'type' : 'market_order', 'trader' : order.trader, 'exchange' : self.name})
            # add to trades_container some amount of match from fullfill 
        elif order.order_type == 'ask':
            prev_assets = order.trader.assets
            prev_cash = order.trader.cash
            order = self.order_book['bid'].fulfill(order, t_cost)
            ...
            if traded_amount > 0:
                traded_price = (next_cash - prev_cash) / traded_amount
                self.exchnange_trades_history.append({'qty': traded_amount, 'price' : traded_price, 'side' : 'sell', 'type' : 'market_order', 'trader' : order.trader, 'exchange' : self.name})
        return order
    ...
    def plot_order_book(self):
        ...
        plt.step(bid_prices, bid_quantities, label='Bids', where='mid')
        plt.step(ask_prices, ask_quantities, label='Asks', where='mid')

        plt.title(f"Order Book for {self.name}")
        plt.xlabel('Price')
        plt.ylabel('Quantity')
        plt.legend()
        plt.show()